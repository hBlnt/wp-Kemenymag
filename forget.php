...
switch ($method) {
    ...
    case "post":
        if (!empty($token) and strlen($token) === 40) {
            ...
            $sqlUser = "UPDATE user SET forgotten_password_token = '', forgotten_password_expires = '', user_password = :resetPassword
            WHERE binary forgotten_password_token = :token AND forgotten_password_expires>now() AND active = 1 AND email = :email";

            $sqlTrainer = "UPDATE trainer SET forgotten_password_token = '', forgotten_password_expires = '', trainer_password = :resetPassword
            WHERE binary forgotten_password_token = :token AND forgotten_password_expires>now() AND active = 1 AND email = :email";
